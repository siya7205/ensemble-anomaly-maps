...
def make_features(traj):
    ...
    X = np.column_stack([rmsd, rg, contacts, sc(phi), sc(psi)])
    return X, phi, psi
...
# ---- Anomaly scoring using Deeptime ----
from deeptime.decomposition import TICA
from deeptime.clustering import KMeans
from deeptime.markov.msm import MaximumLikelihoodMSM

def msm_anomaly(X, lag_tica=10, lag_msm=30, n_clusters=25):
    # Fit TICA (no kinetic_map arg here)
    ...
    # Compute rarity, surprise, density
...
for xtc in xtcs:
    ...
    with open(out / "SUMMARY.txt", "w") as f:
        f.write(f"{name}: frames={len(scores)}, msm_states={msm.n_states}, events phi={len(events['phi'])}, psi={len(events['psi'])}, dssp={'NA' if dssp is None else len(events['dssp'])}; agreement {prec}\n")
